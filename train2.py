...
from utils.mask_utils import DINOFinetune_FeatureExtractor, calculate_residual_mask, interpolation
from utils.mask_utils import MLPModel
import uuid
from tqdm import tqdm
from utils.image_utils import psnr, resize_mask_nearest_hw
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    if not opt.disable_mask:
        feature_extractor = DINOFinetune_FeatureExtractor().cuda()
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        filtered_mask = viewpoint_cam.get_filtered_mask(os.path.join(args.source_path, args.filtered_masks, image_name)).cuda()
        filtered_mask = resize_mask_nearest_hw(filtered_mask, image).float()
        ...
        if not opt.disable_mask and iteration >= opt.mask_beginning and (not((iteration>reset_start) and (iteration<reset_end) and (iteration>=opt.reset_iter))):
            ...
            #reg_loss += 2.0 * (filtered_mask-mask).mean()
            reg_loss += 2 * torch.abs(filtered_mask-mask).mean() * math.exp(-iteration / opt.beta_reg)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[500, 1000, 2000, 3000, 4000, 5000, 6000, 7_000, 10000, 15000, 20000, 30_000])
