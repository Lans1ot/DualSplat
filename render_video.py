...
def interpolate_camera_list(camera_list, n_frames, tension=0.0, bias=0.0, continuity=0.0):
    ...
    for i in range(n_frames):
        ...
        new_camera = Camera(
            ...
            FoVx=FoVx,
            FoVy=FoVy,
            image=to_pil_image(original_camera.original_image), 
            #image=ToPILImage()(original_camera.original_image), 
            ...
            depth_params=None,
            need_resize=False,
            resolution=original_camera.original_image.shape[-2:]
        )
...
def images_to_video(image_folder, output_video_path, fps=30):
    ...
    #     images,
    #     fps=fps,
    #     plugin='ffmpeg',        # 强制用 ffmpeg
    #     codec='libx264',        # 常用 H.264 编码
    #     quality=8,              # 画质（0-10，越大越好）
    #     macro_block_size=None   # 避免自动对齐导致缩放
    # )
...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        rendering = render(
            view, gaussians, pipeline, background)["render"]
        gt = view.original_image[0:3, :, :].to(rendering.device)
        #rendering = torch.concatenate([rendering, gt], dim=-1)
