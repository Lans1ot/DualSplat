...
from utils.camera_utils import calc_voxel_size
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for i in range(gaussiansN):
        ...
        GsDict[f"gaussians{i}"].init_distance_gated_voxel_pruning_from_cameras(GsDict[f"scene{i}"].getTrainCameras())
        # GsDict[f"voxel_size{i}"] = calc_voxel_size(GsDict[f"scene{i}"].getTrainCameras())
        GsDict[f"voxel_size{i}"] = args.voxel_size
        print("Using voxel size: ", GsDict[f"voxel_size{i}"])
        ...
        with torch.no_grad():
            ...
            for i in range(gaussiansN):
                ...
                if iteration < opt.densify_until_iter:
                    ...
                    if iteration >= opt.remove_begin and iteration % opt.remove_interval == 0:
                        print(GsDict[f"gaussians{i}"].prune_random_fixed_per_voxel_distance_gated(voxel_size=GsDict[f"voxel_size{i}"], remove_per_voxel=opt.remove_per_voxel))
...
if __name__ == "__main__":
    ...
    print("train mlp v2 with train.py v3 with voxel filter")
