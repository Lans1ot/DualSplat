...
from utils.mask_utils import DINOFeatureExtractor, calculate_residual_mask, interpolation
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        filtered_mask = resize_mask_nearest_hw(filtered_mask, image)
        if opt.disable_mask or iteration < opt.mask_beginning:
            ...
            # output_path = os.path.join("./generated_mask_", args.source_path.split('/')[-1])
        ...
        if not opt.disable_mask and iteration >= opt.mask_beginning and (not((iteration>reset_start) and (iteration<reset_end) and (iteration>=opt.reset_iter))):
            ...
            reg_loss += 5.0 * ((filtered_mask-mask) * math.exp(-iteration / opt.beta_reg)).mean()
